...
async function seedDashboard() {
  ...
  const tasks = [
    { type: 'kyc_review', title: 'Review KTP for Budi Santoso', description: 'ID Verification', refId: '123', priority: 'normal', status: 'pending', createdAt: admin.firestore.FieldValue.serverTimestamp() },
    { type: 'dispute_mediation', title: 'Resolve dispute TRX-9884', description: 'Mediation', refId: 'TRX-9884', priority: 'urgent', status: 'in_progress', createdAt: new Date(Date.now() - 2 * 3600000) },
    { type: 'maintenance', title: 'Update system config', description: 'Maintenance', refId: null, priority: 'normal', status: 'done', createdAt: new Date(Date.now() - 4 * 3600000) },
    { type: 'report', title: 'Weekly report generation', description: 'Automated', refId: null, priority: 'normal', status: 'done', createdAt: new Date(Date.now() - 12 * 3600000) },
  ];
...
seedDashboard().catch((err) => {
  console.error('❌ Gagal melakukan seeding dashboard:', err);
  process.exit(1);
});
